...
public class LineUserProfileComponent : ViewComponent
{
    ...
    public LineUserProfileComponent(IHttpContextAccessor httpContext, WebServerDBContext webServerDB)
    {
        ...
        try
        {
            ...
            if (httpContext != null)
            {
                ...
                if (user.Identity.IsAuthenticated)
                {
                    ...
                    if (userIdClaim != null) 
                        // 從資料庫中查找LINE使用者資料
                        userProfile = await _webServerDB.LINEUser.FindAsync(userIdClaim.Value);
                }
            }
        }
        catch (Exception ex)
